...
class PlateIndexer():
    ...
    def reconstruct_plate_pile(self):
        # exdict = {'flower-decorated plate': 0,
        #           'light green plate': 2,
        #           'big blue plate': 5,
        ...
        # print(list(exdict.values()))
        # print(list(exdict.values())[1:])

        # exls = list(exdict.values()) + [num_plates]
        # diff_list = map(sub, exls[1:], exls)
        # diff_keys = list(exdict.keys())

        # print(list(map(lambda a, b: b - a, exls, exls[1:])))

        # plates = []
        # rest_vals = list(self.plate_indexes.values)
        # # for k, v in self.plates_indexes.items():
        # #     if v
...
def main():
    ...
    print(plate_indexer.plate_indexes)

    insertion_details = plate_indexer.get_insertion_details(
        "big blue plate"
    )

    print(f"actual insertion details: {insertion_details} stack: {plate_indexer.plate_indexes}")
